...
export class FoldersService {
  ...
  public create(payload: CreateFolderRequest): Observable<IFolder> {
    return this.http
      .post<IFolder>('/api/folders', payload)
      .pipe(tap((folder) => this.foldersSignal.update((folders) => [...folders, folder])));
  }

  public delete(id: string): Observable<void> {
    return this.http
      .delete<void>(`/api/folders/${id}`)
      .pipe(tap(() => this.foldersSignal.update((folders) => folders.filter((f) => f.id !== id))));
  }
}
